...
class DDPGMetrics:
    ...
    def plot_losses(self, ax=None):
        ...
        if self.smooth:
            ...
            self.policy_losses['upper'] = self.smooth_data(self.policy_losses['upper'].to_numpy(), window)
            self.policy_losses['lower'] = self.smooth_data(self.policy_losses['lower'].to_numpy(), window)
            self.q_losses['mean'] = self.smooth_data(self.q_losses['mean'].to_numpy(), window)
            self.q_losses['upper'] = self.smooth_data(self.q_losses['upper'].to_numpy(), window)
            self.q_losses['lower'] = self.smooth_data(self.q_losses['lower'].to_numpy(), window)
            self.episodic_returns['mean'] = self.smooth_data(self.episodic_returns['mean'].to_numpy(), window)
            self.episodic_returns['upper'] = self.smooth_data(self.episodic_returns['upper'].to_numpy(), window)
            self.episodic_returns['lower'] = self.smooth_data(self.episodic_returns['lower'].to_numpy(), window)
        ...
        ax[2].fill_between(x = range(len(n_episodes)),
                        y1=self.episodic_returns['lower'],
                        y2=self.episodic_returns['upper'],
                        alpha=0.2)
