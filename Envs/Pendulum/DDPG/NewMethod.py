        ...
        for t in progress_bar:
            if t < warm_up:
                ...
                if EXPERT == 'Expert':
                    ...
                    if REWARD_TYPE == 'sparse':
                        _, r_bpol, trun_b, term_b, _ = env.step(obs, action_bh)
                        # done_b = trun_b or term_b 
                    else:
                        _, r_bpol, trun_b, term_b, _ = env.step(action_bh)
                        # done_b = trun_b or term_b 
            ...
            if REWARD_TYPE == 'sparse':
                obs_, reward, term, truncated, _ = env.step(obs, clipped_action)
            else:
                obs_, reward, term, truncated, _ = env.step(clipped_action)
